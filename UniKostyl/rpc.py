...
class rpc:
    ...
    def __tab_crc_16(self, data, size):  # private
        crc = 0xFFFF
        for i in range(size):
            crc = self.__crc_16_table[((crc >> 8) ^ data[i]) & 0xFF] ^ (crc << 8)
        return crc & 0xFFFF
    ...
    def _set_packet(self, magic_value, payload=bytes()):  # private
        ...
        new_payload[-2:] = struct.pack(
            "<H", self.__crc_16(new_payload, len(payload) + 2)
        )
        return new_payload
    ...
    def stream_writer(self, call_back, write_timeout_ms=5000):  # public
        ...
        queue_depth = max(
            min(
                ...
                self._stream_writer_queue_depth_max,
            ),
            1,
        )
        rx_lfsr = 255
...
class rpc_master(rpc):
    ...
    def __put_command(self, command, data, timeout):  # private
        ...
        while (int(time.time() * 1000) - start) < timeout:
            ...
            self._zero(
                self.__in_command_header_buf[0], len(self.__in_command_header_buf[0])
            )
            self._zero(
                self.__in_command_data_buf[0], len(self.__in_command_data_buf[0])
            )
            self._flush()
    ...
    def __get_result(self, timeout):  # private
        ...
        while (int(time.time() * 1000) - start) < timeout:
            gc.collect()  # Avoid collection during the transfer.
            self._zero(
                self.__in_result_header_buf[0], len(self.__in_result_header_buf[0])
            )
            self._flush()
...
class rpc_slave(rpc):
    ...
    def __put_result(self, data, timeout):  # private
        ...
        while (int(time.time() * 1000) - start) < timeout:
            gc.collect()  # Avoid collection during the transfer.
            self._zero(
                self.__in_response_header_buf[0], len(self.__in_response_header_buf[0])
            )
            self._zero(
                self.__in_response_data_buf[0], len(self.__in_response_data_buf[0])
            )
            self._flush()
            if (
                self._get_packet(
    ...
    def loop(self, recv_timeout=1000, send_timeout=1000):  # public
        while True:
            ...
            if command is not None:
                ...
                if (
                    self.__put_result(
                        cb(data) if cb is not None else bytes(), send_timeout
                    )
...
class rpc_kvarser_can_master(rpc_master):
    ...
    def __init__(
        self, channel, message_id=0x7FF, bit_rate=250000, sampling_point=75
    ):  # private
    ...
    def put_bytes(self, data, timeout_ms):  # protected
        ...
        for i in range(0, len(view), 8):
            try:
                self.__can.writeWait(
                    Frame(id_=self.__message_id, data=view[i : i + 8]),
