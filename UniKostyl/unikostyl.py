import io, pygame, sys, time, ctypes, os
import numpy as np
from widgets import *
import cv2
...
class Thresholds_t(ctypes.Structure):
    _fields_ = [("Lmin", ctypes.c_int), ("Lmax", ctypes.c_int),
...
def threshold_from_area(rect, pixels):
    # rect - это [x1, y1, w, h] в координатах массива
...
try:
    # Пытаемся загрузить и отрисовать картинку
    splash = pygame.image.load(image_path)
    splash = pygame.transform.scale(splash, (screen_w, screen_h))
    screen.blit(splash, (0, 0))
    pygame.display.flip()
    print("Заставка загружена успешно")
except Exception as e:
    # Если картинки нет, просто закрасим серым, чтобы не было черного экрана
    screen.fill((100, 100, 100))
    pygame.display.flip()
    print(f"Заставка не найдена, используем фон по умолчанию. Ошибка: {e}")
...
def update_all_widgets():
    global screen_w, screen_h, widgets
    ...
    mode_start_x = margin + 180
    sync_btn('btn_bitmap', mode_start_x, top_y, 90, btn_h)
    sync_btn('btn_l', mode_start_x + 100, top_y, 40, btn_h)
    sync_btn('btn_a', mode_start_x + 150, top_y, 40, btn_h)
    sync_btn('btn_b', mode_start_x + 200, top_y, 40, btn_h)
    sync_btn('btn_pause', mode_start_x + 260, top_y, 50, btn_h)

    # --- 2. КАМЕРЫ (СМЕЩЕНЫ ВЛЕВО) ---
    v_y = top_y + btn_h + 20
    gap = 6 # Зазор между окнами

    # Расчет максимально возможной стороны квадрата
    # Теперь считаем от всей ширины, деленной на 2
    max_v_w = (screen_w - (margin * 2) - gap) // 2
    max_v_h = int(screen_h * 0.65)
    side = min(max_v_w, max_v_h)

    # Прижимаем левое окно к маргину
    x_left = margin
    # Правое окно ставим сразу за левым
    ...
    for i, item in enumerate(widgets['itemlist_thr'].items):
        if i < 2:
            ...
            item.rect = pygame.Rect(margin + col_text_w + 30, list_y + (i-2)*20, col_text_w, 20)
...
# ВАЖНО: Слайдеры в словаре идут ПОСЛЕ подложек, чтобы в цикле [::-1] они были ПЕРВЫМИ
widgets = {
...
def main_loop_frame(image_pixels: np.array):
    global screen_w, screen_h, screen, widgets, pixels_LAB, thr_index, process_mode, thresholds, is_pause
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONUP:
            # Сначала забираем данные выделения
            ...
            if had_press and temp_area:
                img_r = src_w.rect; aw, ah = pixels_LAB.shape[0], pixels_LAB.shape[1]
                sx, sy = aw / img_r.width, ah / img_r.height
                x1 = int(max(0, (temp_area.left - img_r.left) * sx))
                y1 = int(max(0, (temp_area.top - img_r.top) * sy))
                x2 = int(min(aw - 1, (temp_area.right - img_r.left) * sx))
                y2 = int(min(ah - 1, (temp_area.bottom - img_r.top) * sy))
                if x2 > x1 and y2 > y1:
    ...
    if pixels_LAB.size > 0:
        if process_mode == 'Bitmap': widgets['img_proc'].pixels = threshold_filter(thresholds[thr_index], pixels_LAB)
        elif process_mode == 'L': widgets['img_proc'].pixels = filter_L(thresholds[thr_index], pixels_LAB)
        elif process_mode == 'A': widgets['img_proc'].pixels = filter_A(thresholds[thr_index], pixels_LAB)
        elif process_mode == 'B': widgets['img_proc'].pixels = filter_B(thresholds[thr_index], pixels_LAB)
...
set_thr_to_sliders(); update_all_widgets()
